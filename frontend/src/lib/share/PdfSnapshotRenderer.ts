...
export function renderSnapshotPdf(snapshot: GeneratedCardSnapshot): { buffer: Buffer; filename: string } {
  ...
  const createdAt = text(snapshot.created_at) || text((snapshot as any).generatedAt) || new Date().toISOString()
  const cardVersion = text(snapshot.card_version) || '1.0'
  const title = snapshot.header_subject || text((snapshot as any).title) || snapshot.canonical_question || 'Situation Card'
  const domain = snapshot.header_domain || text((snapshot as any).domain)
  const submitted =
    snapshot.situation_soumise ||
    text((snapshot as any).submittedQuestion) ||
  ...
  const sections: PdfSection[] = [
    ...
    { title: 'Traçabilité', body: `${isFr ? 'Généré par IAAA+ SIS' : 'Generated by IAAA+ SIS'} · ${createdAt.slice(0, 10)}\nVersion ${cardVersion} · Lecture structurelle`, tone: 'muted' },
    ...
    { title: isFr ? 'Provenance' : 'Provenance', body: `${cardVersion} - ${createdAt} - snapshot ${snapshot.id}`, tone: 'muted' },
