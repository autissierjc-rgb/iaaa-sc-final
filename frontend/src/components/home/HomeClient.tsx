...
function ScoreBar({ score }: { score: number }) {
  return (
    <div style={{ display: 'flex', gap: 2 }}>
      {[1, 2, 3].map(i => (
        <div key={i} style={{ width: 14, height: 6, borderRadius: 2, background: score >= i ? '#F0CA70' : '#E0DCD4', transition: 'background .3s' }} />
      ))}
    </div>
  )
}

function SituationCardPanel({ sc, onExpand }: { sc: any; onExpand: () => void }) {
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '10px 14px' }}>
        ...
        {sc.trajectories && (
          <div style={{ marginBottom: 10 }}>
            ...
            {sc.trajectories.map((t: any, i: number) => (
      ...
      {scExpanded && scData && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: BG_P, borderRadius: 12, width: '100%', maxWidth: 720, maxHeight: '90vh', display: 'flex', flexDirection: 'column', border: `1px solid ${BDR_G}` }}>
            <SituationCardPanel sc={scData} onExpand={() => setScExpanded(false)} />
          </div>
        </div>
      )}
            ...
            <div style={{ background: BG_P, border: `1px solid ${BDR}`, borderRadius: 16, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 16px 12px', borderTop: `1px solid ${BDR}` }}>
                ...
                <button
                  ...
                  disabled={!isTyping || scLoading}
                  style={{
                    width: 44, height: 44, borderRadius: '50%', border: 'none', cursor: isTyping ? 'pointer' : 'not-allowed',
                    padding: 0, overflow: 'hidden', background: 'none',
                    opacity: isTyping ? 1 : 0.4,
                    animation: isTyping && !scLoading ? 'compassPulse 2s ease-in-out infinite' : 'none',
                  }}
              ...
              {!scVisible ? (
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 12 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, marginTop: 16, marginBottom: 6 }}>
                    ...
                    <span style={{ color: TXT2 }}>{lang === 'FR' ? 'En attente' : 'Waiting'}</span>
                  </div>
                  <div style={{ fontSize: 11, color: TXT3, fontStyle: 'italic', fontFamily: "'Cormorant Garamond', serif" }}>
                    {lang === 'FR' ? 'Décrivez une situation et cliquez la boussole' : 'Describe a situation and click the compass'}
                  </div>
                </div>
              ) : scLoading ? (
                /* Chargement */
                ...
                <div style={{ flex: 1, overflow: 'hidden' }}>
                  <SituationCardPanel sc={scData} onExpand={() => setScExpanded(true)} />
                </div>
              ) : null}
            </div>
          </div>
