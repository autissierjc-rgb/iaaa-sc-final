...
function AstrolabeRadial({ scores }: {
  ...
  return (
    <svg viewBox="0 0 220 220" width="200" height="200">
      {[28, 56, 84].map(rr => (
        <circle key={rr} cx={cx} cy={cy} r={rr}
          fill="none" stroke={BDR_G} strokeWidth="0.7" strokeDasharray="3 3" />
      ))}
      {scores.map((s, i) => {
        const a = (i * 45 - 90) * Math.PI / 180
        const xEnd = cx + r * Math.cos(a)
        const yEnd = cy + r * Math.sin(a)
        const sr = s.display_score * 28
        const xs = cx + sr * Math.cos(a)
        const ys = cy + sr * Math.sin(a)
        const col = COLORS[s.display_score] ?? COLORS[0]
        const xl = cx + (r + 14) * Math.cos(a)
        const yl = cy + (r + 14) * Math.sin(a)
        return (
          <g key={i}>
            <line x1={cx} y1={cy} x2={xEnd} y2={yEnd}
              stroke={BDR_G} strokeWidth="0.7" />
            {s.display_score > 0 && (
              <line x1={cx} y1={cy} x2={xs} y2={ys}
                stroke={col.stroke} strokeWidth="2" opacity="0.5" />
            )}
            <polygon
              points={`${cx},${cy - 5} ${xs - 4},${ys} ${cx},${cy + 5} ${xs + 4},${ys}`}
              fill={col.fill} stroke={col.stroke} strokeWidth="0.8" opacity="0.9"
            />
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
            ...
            {sc.radar_scores?.length > 0 && (
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 6 }}>
                  <span style={{ fontSize: 32, fontWeight: 700, color: TXT, fontFamily: "'Cinzel',serif" }}>
                    {sc.state_index_final}
                  </span>
                  <span style={{ fontSize: 11, color: TXT3 }}>/ 100 · {stateLabel}</span>
                </div>
                ...
                {sc.analysis?.avertissement && (
                  <div style={{ fontSize: 10, color: TXT3, fontStyle: 'italic', lineHeight: 1.5 }}>
      ...
      <div style={{
        ...
        <button style={{
          ...
          padding: '6px 8px', fontSize: 10, fontFamily: "'Cinzel',serif",
      ...
      <div style={{ minHeight: '100vh', background: BG }}>
        ...
        <main style={{ maxWidth: 1160, margin: '0 auto', padding: '24px 28px 0' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '16px 0 14px' }}>
            ...
            <div style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, fontStyle: 'italic', color: GOLD_L }}>
              {t.tagline}
            </div>
          </div>
        </main>
