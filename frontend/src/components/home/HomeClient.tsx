...
function SituationCardPanel({ sc, lang, onExpand }: {
  ...
  const radarScores: any[] = sc.radar_scores ?? (sc.radar ? [
    ...
    { dimension: 'Incertitude',   dimension_en: 'Uncertainty',   score: Math.round(sc.radar.uncertainty / 33) },
            ...
            {trajectories.length > 0 && (
              <div>
                ...
                {trajectories.map((tr: any, i: number) => {
                  ...
                  const lbl  = lang === 'FR' ? (tr.type_fr ?? tr.type ?? '') : (tr.type_en ?? tr.type ?? '')
      ...
      <div style={{ borderTop: `1px solid ${BDR}`, padding: '8px 14px', display: 'flex', gap: 8 }}>
        ...
        <button style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, background: 'transparent', color: TXT2, border: `1px solid ${BDR}`, borderRadius: 7, padding: '7px 8px', fontSize: 11, fontFamily: "'Cinzel',serif", letterSpacing: '.04em', cursor: 'pointer' }}>
          <div style={{ width: 14, height: 14, overflow: 'hidden', borderRadius: 2, flexShrink: 0 }}>
            <Image src="/pictos/cartes.jpeg" alt="" width={14} height={14} style={{ objectFit: 'cover', width: '100%', height: '100%' }} unoptimized />
          </div>
          {TX[lang].lectures}
        </button>
        <button style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, background: 'transparent', color: TXT2, border: `1px solid ${BDR}`, borderRadius: 7, padding: '7px 8px', fontSize: 11, fontFamily: "'Cinzel',serif", letterSpacing: '.04em', cursor: 'pointer' }}>
...
export default function HomeClient() {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      // ── GENERATE — astrolabe (haiku ~3s) ET SC (sonnet ~25s) en parallèle ──
      // Les deux fetch sont lancés simultanément, astrolabe affiché dès réception
