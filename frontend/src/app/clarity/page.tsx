...
/**
 * IAAA · /clarity
 * Bouton 1 de la home — compréhension individuelle
 */
...
const EXAMPLES = [
  { tag: 'Personnel', tagColor: '#185FA5', text: 'Je me sépare. Je ne sais pas si c\'est la bonne décision.' },
  ...
  { tag: 'Professionnel', tagColor: '#C8951A', text: 'Mon équipe fonctionne mais quelque chose cloche. Je n\'arrive pas à mettre le doigt dessus.' },
]
    ...
    <main style={{ background: '#F5F0E8', minHeight: '100vh', fontFamily: 'var(--font-dm-sans, system-ui, sans-serif)', color: '#1A2E5A' }}>
      ...
      <div style={{ padding: '52px 36px 0' }}>
        ...
        <p style={{ fontSize: '14px', color: '#7A6A5A', lineHeight: 1.8, maxWidth: '440px', marginBottom: '28px' }}>
          Décrivez une situation. La carte en révèle la structure, les tensions et les évolutions possibles.
        </p>
      ...
      <div style={{ background: '#fff', padding: '44px 36px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '36px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '11px' }}>
          {['Une lecture structurée de votre situation', 'La vulnérabilité centrale', 'Trois évolutions possibles', 'Un signal concret à surveiller'].map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', fontSize: '13px', color: '#5A6A7A', lineHeight: 1.6 }}>
              <span style={{ color: '#C8951A', fontSize: '10px', marginTop: '3px', flexShrink: 0 }}>◆</span>{item}
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: '#F5F0E8', borderTop: '1px solid #E8E0D0', padding: '36px 36px' }}>
        <div style={{ maxWidth: '480px', margin: '0 auto', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '13px', color: '#9A8860', marginBottom: '20px', lineHeight: 1.7 }}>
            Clarity est libre d'accès. Créez votre première Situation Card en moins d'une minute.
          </p>
      ...
      <footer style={{ borderTop: '1px solid #E8E0D0', padding: '16px 36px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#F5F0E8' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', fontSize: '12px', color: '#9A8860' }}>
          <Link href="/sis"     style={{ color: '#9A8860', textDecoration: 'none' }}>Voir SIS →</Link>
          <Link href="/pricing" style={{ color: '#9A8860', textDecoration: 'none' }}>Pricing →</Link>
        </div>
      </footer>
